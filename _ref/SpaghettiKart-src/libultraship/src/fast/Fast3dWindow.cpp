...
void Fast3dWindow::Init() {
    ...
    width = Ship::Context::GetRawInstance()->GetConfig()->GetInt("Window.Width", x);
    height = Ship::Context::GetRawInstance()->GetConfig()->GetInt("Window.Height", y);
#endif
...
#ifdef _UWP
    return std::min<uint32_t>(std::ceil(uwp_GetRefreshRate()), 60);
#else
