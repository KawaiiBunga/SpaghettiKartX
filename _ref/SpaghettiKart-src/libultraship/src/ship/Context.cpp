...
#ifdef _UWP
extern "C" const char* uwp_get_aux_root();
#endif
...
bool Context::InitLogging(spdlog::level::level_enum debugBuildLogLevel,
    ...
    try {
        ...
        std::error_code ecLog;
        std::filesystem::create_directories(std::filesystem::path(logPath).parent_path(), ecLog);
        try {
        ...
        try {
            ...
            mLogger = std::make_shared<spdlog::logger>("fallback", nullSink);
            spdlog::register_logger(GetLogger());
            spdlog::set_default_logger(GetLogger());
        }
        return true;
...
#if defined(_UWP)
    const char* aux = uwp_get_aux_root();
    if (aux && aux[0] != '\0') {
        return std::string(aux);
    }
    char* prefpath = SDL_GetPrefPath(NULL, "SpaghettiKart");
    if (prefpath != NULL) {
