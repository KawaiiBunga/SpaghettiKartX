...
#ifdef _UWP
static std::string g_uwpAuxRoot;

extern "C" __declspec(dllexport) void uwp_set_aux_root(const char* root) {
    if (root) g_uwpAuxRoot = root;
}

extern "C" __declspec(dllexport) const char* uwp_get_aux_root() {
    if (!g_uwpAuxRoot.empty()) {
        return g_uwpAuxRoot.c_str();
    }
    char* pref = SDL_GetPrefPath(NULL, "SpaghettiKart");
    if (pref) {
        g_uwpAuxRoot = pref;
        SDL_free(pref);
        return g_uwpAuxRoot.c_str();
...
#elif defined(__SWITCH__) || defined(_UWP)
    SPDLOG_ERROR(box);
    return IDNO;
#else
