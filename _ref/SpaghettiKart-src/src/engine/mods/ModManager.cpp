...
#ifdef _UWP
    SPDLOG_ERROR("mk64.o2r not found in app directory or bundle!");
    return;
#else
...
std::vector<std::string> ListMods() {
    std::string main_path = Ship::Context::LocateFileAcrossAppDirs(game_asset_file);
    if (!std::filesystem::exists(main_path)) {
        main_path = Ship::Context::GetPathRelativeToAppDirectory(game_asset_file);
    }
    ...
    std::error_code ec;
    if (!std::filesystem::exists(mods_path, ec)) {
        std::filesystem::create_directories(mods_path, ec);
        if (!ec) {
